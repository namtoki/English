# Hedge

## VAGUE (`a kind of` / all kinds of / `and things like that`)
> `a kind of`                       ([Neutral] 一種の)              It's a kind of fruit.
  - `a sort of`                     ([Neutral] 一種の)              He felt a sort of sadness.
  - `a type of`                     ([Neutral] 一種の、タイプの)    This is a type of software.
> `all kinds of`                    ([Casual] あらゆる種類の)       I eat all kinds of food.
  - `all sorts of`                  ([Casual] あらゆる種類の)       She reads all sorts of books.
  - `all kinds of things`           ([Neutral] 色々なもの)          They sell all kinds of things.
  - `various kinds of`              ([Neutral] 様々な種類の)        Various kinds of flowers bloom here.
> `... and things like that`        ([Neutral] 〜などそういうもの)  Spanish and things like that.
  - `... and stuff like that`       ([Casual] 〜とかそういうの)     Budgets, deadlines, and stuff like that.
  - `... and things` /ænd θɪŋz/     ([Casual] とかいろいろ)         She bought shoes, bags, and things.
  - `... and stuff` /ænd stʌf/      ([Casual] とかそういうの)       He likes pizza, burgers, and stuff.
  - `... and all that jazz`         ([Casual] そういうもの全部)     He talks about success, money, and all that jazz.
  - `... and all that` /ænd ɔːl ðæt/([Casual] などなど、~とか)      He likes movies, books, and all that.
  - `... and so forth` /ænd soʊ fɔrθ([Neutral] などなど)            First, second, third, and so forth.
  - `... and so on` /ænd soʊ ɑn/    ([Neutral] というように)        One, two, three, and so on.
  - `... and such` /ænd sʌtʃ/       ([Neutral] そういったもの)      He studies math, science, and such.
  - `... and the like` /ænd ðə laɪk/([Neutral] そういうもの)        We need books, papers, and the like.
  - `... and whatnot` /ænd ˈwʌtnɑt/ ([Casual] など)                 She collects stamps, coins, and whatnot.
  - `... et cetera`                 ([Formal] 〜など)               Books, magazines, et cetera.
  - `... etc.`                      ([Neutral] 〜など)              Apples, oranges, etc.

## AMOUNT (`as little as` / `scant` / a few / several / significant / a lot of / rich / `as many as`)
> `as little as`                    ([Neutral] わずか)              It costs as little as $10.
  - `as few as`                     ([Neutral] わずか)              As few as 5 people showed up.
> `scant` /skænt/                   ([Formal] わずかな、乏しい)     There is scant evidence.
  - `scanty` /ˈskænti/              ([Formal] わずかな、不十分な)   The data was scanty.
  - `limited` /ˈlɪmɪtɪd/            ([Neutral] 限られた)            We have limited resources.
  - `minimal` /ˈmɪnɪməl/            ([Neutral] 最小限の)            There was minimal damage.
  - `scarce` /skeərs/               ([Formal] 乏しい、不足)         Food was scarce during winter.
  - `negligible` /ˈneɡlɪdʒəbəl/     ([Formal] 無視できる程わずか)   The risk is negligible.
  - `meager` /ˈmiːɡər/              ([Formal] わずかな、乏しい)     He earns a meager salary.
  - `paltry` /ˈpɔːltri/             ([Formal] わずかな、些細な)     She received a paltry sum.
> `a few`                           ([Neutral] 少数の)              I have a few questions.
  - `a handful of`                  ([Neutral] 少数の)              Only a handful of people came.
  - `a small number of`             ([Neutral] 少数の)              A small number of issues remain.
  - `a smattering of`               ([Formal] 少数の、わずかな)     A smattering of applause followed.
  - `a sprinkling of`               ([Neutral] わずかな、少数の)    A sprinkling of students came early.
  - `a little more`                 ([Neutral] もう少し)            Just a little more patience.
  - `a bit more`                    ([Casual] もう少し)             I need a bit more time.
> `several` /ˈsevrəl/               ([Neutral] いくつかの、数個の)  Several people attended.
  - `a couple of`                   ([Casual] いくつかの)           I need a couple of minutes.
  - `some` /sʌm/                    ([Neutral] いくつかの)          Some students were absent.
  - `various` /ˈveriəs/             ([Neutral] 様々な、いくつかの)  Various options are available.
  - `multiple` /ˈmʌltɪpəl/          ([Neutral] 複数の)              Multiple issues were found.
> `significant` /sɪɡˈnɪfɪkənt/      ([Neutral] かなりの、重要な)    There was significant improvement.
  - `decent` /ˈdiːsənt/             ([Casual] かなりの、まともな)   He earns a decent salary.
  - `substantial` /səbˈstænʃəl/     ([Formal] かなりの、相当な)     They made substantial progress.
  - `moderate` /ˈmɑːdərət/          ([Neutral] 適度な、ほどほどの)  She has moderate experience.
  - `a fair amount of`              ([Neutral] かなりの)            A fair amount of work remains.
  - `a good deal of`                ([Neutral] かなりの)            A good deal of time was spent.
  - `a considerable amount of`      ([Formal] かなりの)             A considerable amount of money was spent.
  - `sizable` /ˈsaɪzəbəl/           ([Neutral] かなりの大きさの)    A sizable portion was allocated.
  - `appreciable` /əˈpriːʃəbəl/     ([Formal] かなりの、相当な)     An appreciable difference exists.
> `a lot of`                        ([Neutral] たくさんの)          A lot of people came.
  - `many` /ˈmeni/                  ([Neutral] 多くの)              Many students participated.
  - `lots of`                       ([Casual] たくさんの)           There are lots of options.
  - `plenty of`                     ([Casual] たくさんの、十分な)   There's plenty of time.
  - `a bunch of`                    ([Casual] たくさんの)           A bunch of people came.
  - `tons of`                       ([Casual] たくさんの)           We have tons of work to do.
  - `loads of`                      ([Casual] たくさんの)           She has loads of experience.
  - `a number of`                   ([Formal] 多数の)               A number of factors were considered.
  - `numerous` /ˈnuːmərəs/          ([Formal] 多数の、数多くの)     Numerous attempts were made.
  - `a great deal of`               ([Formal] 大いなる)             A great deal of effort was required.
  - `countless` /ˈkaʊntləs/         ([Formal] 無数の)               Countless hours were spent.
  - `a huge amount of`              ([Casual] 膨大な)               A huge amount of time was wasted.
  - `an enormous amount of`         ([Neutral] 膨大な)              An enormous amount of data was collected.
  - `a vast amount of`              ([Formal] 膨大な)               A vast amount of research was done.
  - `a multitude of`                ([Formal] 多数の、たくさんの)   A multitude of issues arose.
> `rich` /rɪtʃ/                     ([Neutral] 豊富な、豊かな)      The area is rich in resources.
  - `abundant` /əˈbʌndənt/          ([Neutral] 豊富な)              Food was abundant at the party.
  - `abundance` /əˈbʌndəns/         ([Neutral] 豊富、大量)          There's an abundance of opportunities.
  - `ample` /ˈæmpəl/                ([Neutral] 十分な、豊富な)      There's ample time to finish.
  - `a wealth of`                   ([Neutral] 豊富な)               She has a wealth of experience.
  - `plentiful` /ˈplentɪfəl/        ([Neutral] 豊富な、十分な)      Food was plentiful.
  - `plenty` /ˈplenti/              ([Neutral] たくさん、豊富)      There's plenty of time.
  - `copious` /ˈkoʊpiəs/            ([Formal] 豊富な、大量の)       He took copious notes.
  - `generous` /ˈdʒenərəs/          ([Neutral] 豊富な、気前の良い)  A generous amount was provided.
  - `bountiful` /ˈbaʊntɪfəl/        ([Formal] 豊富な、気前の良い)   The harvest was bountiful.
  - `profuse` /prəˈfjuːs/           ([Formal] 豊富な、あふれる)     Profuse apologies were given.
  - `lavish` /ˈlævɪʃ/               ([Formal] 豊富な、贅沢な)       They received lavish praise.
> `as many as`                      ([Neutral] 〜も)                As many as 1000 people came.
  - `as much as`                    ([Neutral] 〜も)                It costs as much as $500.
