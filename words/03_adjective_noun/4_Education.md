# Education

## Capacity (無知・教養)
> `ignorance` /ˈɪɡnərəns/           ([Neutral] 無知、無学)          His ignorance of basic facts was shocking.
  - `ignorant` /ˈɪɡnərənt/          ([Neutral] 無知な、知らない)    He's ignorant of the rules.
  - `uneducated` /ˌʌnˈedʒukeɪtɪd/   ([Neutral] 無教育の、無教養の)  The uneducated masses need support.
  - `clueless` /ˈkluːləs/           ([Casual] 無知な、何も知らない) He's completely clueless about technology.
  - `literacy` /ˈlɪtərəsi/          ([Neutral] 識字能力、教養)      Digital literacy is essential today.
  - `literate` /ˈlɪtərət/           ([Neutral] 読み書きできる)      All citizens should be literate.
  - `illiterate` /ɪˈlɪtərət/        ([Neutral] 読み書きできない)    Many adults are functionally illiterate.
  - `illiteracy` /ɪˈlɪtərəsi/       ([Neutral] 非識字、無教養)      Illiteracy remains a problem in rural areas.
  - `innumeracy` /ɪˈnuːmərəsi/      ([Technical] 数的能力不足)      Innumeracy is as problematic as illiteracy.
  - `innumerate` /ɪˈnuːmərət/       ([Technical] 計算できない)      Many people are innumerate.
  - `benightedness` /bɪˈnaɪtədnəs/  ([Formal] 無知蒙昧)             The benightedness of the era is evident.

## Academic (学問・科目)
> `mathematics` /ˌmæθəˈmætɪks/      ([Neutral] 数学)                Mathematics is the most studied subject.
  - `math` /mæθ/                    ([Casual] 数学)                 I'm good at math.
  - `maths` /mæθs/                  ([British] 数学)                She teaches maths.
  - `arithmetic` /əˈrɪθmətɪk/       ([Neutral] 算数、算術)          Basic arithmetic is essential.
  - `algebra` /ˈældʒɪbrə/           ([Neutral] 代数学)              Algebra helps solve equations.
  - `geometry` /dʒiˈɑːmətri/        ([Neutral] 幾何学)              Geometry studies shapes.
  - `calculus` /ˈkælkjələs/         ([Neutral] 微積分)              Calculus is required for engineering.
> `science` /ˈsaɪəns/               ([Neutral] 科学、理科)          Science class is my favorite.
  - `biology` /baɪˈɑːlədʒi/         ([Neutral] 生物学)              Biology studies living organisms.
  - `chemistry` /ˈkemɪstri/         ([Neutral] 化学)                Chemistry deals with substances.
  - `physics` /ˈfɪzɪks/             ([Neutral] 物理学)              Physics explains natural phenomena.
> `history` /ˈhɪstəri/              ([Neutral] 歴史)                History teaches us about the past.
> `geography` /dʒiˈɑːɡrəfi/         ([Neutral] 地理学)              Geography covers maps and regions.
> `literature` /ˈlɪtərətʃər/        ([Neutral] 文学)                We study literature in English class.
> `philosophy` /fɪˈlɑːsəfi/         ([Neutral] 哲学)                Philosophy explores fundamental questions.
> `economics` /ˌiːkəˈnɑːmɪks/       ([Neutral] 経済学)              Economics studies resource allocation.
  - `economy` /ɪˈkɑːnəmi/           ([Neutral] 経済)                The economy is growing.
> `psychology` /saɪˈkɑːlədʒi/       ([Neutral] 心理学)              Psychology studies human behavior.
> `sociology` /ˌsoʊsiˈɑːlədʒi/      ([Neutral] 社会学)              Sociology examines social structures.

## TEACHING / LEARNING (instruction / instructor)
> `instruction` /ɪnˈstrʌkʃən/       ([Neutral] 指示、指導)          Please follow the instructions carefully.
  - `instructor` /ɪnˈstrʌktər/      ([Neutral] 指導者、教師)        The instructor explained the concept clearly.
  - `teacher` /ˈtiːtʃər/            ([Neutral] 教師、先生)          My teacher is very helpful.
  - `educator` /ˈedʒukeɪtər/        ([Formal] 教育者)               She's a respected educator.
  - `tutor` /ˈtuːtər/               ([Neutral] 個人教師、家庭教師)  I hired a tutor for math.

## ART / LITERATURE (motif / theme / symbol)
> `motif` /moʊˈtiːf/                ([Neutral] モチーフ、主題)      The jacket has a rose motif on the collar.
  - `theme` /θiːm/                  ([Neutral] テーマ、主題)        The theme of the novel is love.
  - `leitmotif` /ˌlaɪtmoʊˈtiːf/    ([Formal] ライトモチーフ)       Freedom is a leitmotif in her work.
  - `symbol` /ˈsɪmbəl/              ([Neutral] 象徴、シンボル)      The dove is a symbol of peace.
  - `element` /ˈelɪmənt/            ([Neutral] 要素)                Suspense is a key element in this story.
